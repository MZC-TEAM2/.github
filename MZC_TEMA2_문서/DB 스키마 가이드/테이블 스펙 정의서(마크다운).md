...
> LMS 데이터베이스 테이블 명세서
> 
> **작성자** : MZC_LMS_TEAM2 (최종수정 2025-12-23)
## 1. 기본 코드 테이블
...
### 16.1 course_notices (강의 공지사항)

| 컬럼명 | 타입 | NULL | Key | 기본값 | 설명 |
|--------|------|------|-----|--------|------|
| id | BIGINT | NO | PK | AUTO_INCREMENT | 공지사항 식별자 |
| course_id | BIGINT | NO | FK, IDX | - | 강의 ID |
| title | VARCHAR(200) | NO | - | - | 공지 제목 |
| content | TEXT | NO | - | - | 공지 내용 |
| allow_comments | BIT(1) | NO | - | 1 | 댓글 허용 여부 |
| created_by | BIGINT | NO | FK, IDX | - | 작성자 ID (교수) |
| updated_by | BIGINT | YES | FK | NULL | 수정자 ID |
| created_at | DATETIME(6) | NO | IDX | NOW() | 생성일시 |
| updated_at | DATETIME(6) | YES | - | NULL | 수정일시 |
| deleted_at | DATETIME(6) | YES | - | NULL | 삭제일시 |
| is_deleted | BIT(1) | NO | IDX | 0 | 삭제 여부 |
...
**Indexes:**
- `idx_course_notice_course_id (course_id)`
- `idx_course_notice_is_deleted (is_deleted)`

**비즈니스 규칙:** 담당 교수만 공지사항 작성/수정/삭제 가능, 수강생은 조회만 가능

### 16.2 course_notice_comments (강의 공지사항 댓글)

| 컬럼명 | 타입 | NULL | Key | 기본값 | 설명 |
|--------|------|------|-----|--------|------|
| id | BIGINT | NO | PK | AUTO_INCREMENT | 댓글 식별자 |
| notice_id | BIGINT | NO | FK, IDX | - | 공지사항 ID |
| parent_id | BIGINT | YES | FK, IDX | NULL | 부모 댓글 ID (대댓글용) |
| content | TEXT | NO | - | - | 댓글 내용 |
| author_id | BIGINT | NO | FK, IDX | - | 작성자 ID |
| created_by | BIGINT | NO | FK, IDX | - | 생성자 ID |
| updated_by | BIGINT | YES | FK | NULL | 수정자 ID |
...
**Foreign Keys:**
- `notice_id` -> `course_notices(id)`
- `parent_id` -> `course_notice_comments(id)` (Self-referencing)
- `author_id` -> `users(id)`
- `created_by` -> `users(id)`
- `updated_by` -> `users(id)`

**Indexes:**
- `idx_course_notice_comment_notice_id (notice_id)`
- `idx_course_notice_comment_parent_id (parent_id)`
- `idx_course_notice_comment_is_deleted (is_deleted)`

**비즈니스 규칙:**
- 댓글은 수강생 및 담당 교수 모두 작성 가능
- 대댓글 지원 (최대 2depth)
- 댓글 수정/삭제는 작성자만 가능
...
[DB_TABLE_UML.pdf](DB_TABLE_UML.pdf)
---
